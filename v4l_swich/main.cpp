...
int main(int argc, char** argv)
{
    ...
    // sink = gst_element_factory_make("nv3dsink", "sink");
     sink = gst_element_factory_make("nvdrmvideosink", "sink");
    g_object_set(sink, "sync", FALSE,"plane-id",1, NULL);
    // sync=true enable-last-sample=false
    identity0 = gst_element_factory_make("queue2","identity0");
    identity1 = gst_element_factory_make("queue2","identity1");

    queue0 = gst_element_factory_make("queue2","queue0");
    queue1 = gst_element_factory_make("queue2","queue1");
    queues = gst_element_factory_make("queue2","queues");


    // g_object_set(queue0,"max-size-buffers",10,NULL);
    // g_object_set(queue1,"max-size-buffers",10,NULL);
    // g_object_set(queues,"max-size-buffers",10,NULL);
    // g_object_set(identity0,"max-size-buffers",10,NULL);
    // g_object_set(identity1,"max-size-buffers",10,NULL);
    ...
    gst_bin_add_many(GST_BIN (pipline),
                    source0,vid_conv0,identity0,queue0,
                    source1,vid_conv1,identity1,queue1,
    ...
    if (!gst_element_link_filtered(source0, queue0, caps_src0)) {
    ...
    if (!gst_element_link_filtered(source1, queue1, caps_src1)) {
        g_printerr("Fail to gst_element_link_filtered videosrc -- vidconv\n");
        return -1;
    }
    ...
    gst_element_link(queue0,vid_conv0);
    gst_element_link(queue1,vid_conv1);
 ...
 */
    if (gst_element_link_many(compositor,queues,sink ,NULL) != TRUE)
    {
        g_printerr("Elements could not be linked.\n");
        gst_object_unref(pipline);
        return -1;
