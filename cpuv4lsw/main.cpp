...
{
    GstElement *pipline; 
    GstElement *source0, *vid_conv0, *identity0, *queue0;
    GstElement *source1, *vid_conv1, *identity1, *queue1;

    GstElement *compositor,*sink_conv,*sink, *queues;
    GstBus *bus;
    GstMessage *msg;
    ...
    // create elements 
    source0 = gst_element_factory_make("v4l2src", "source0");
    ...
    vid_conv0 = gst_element_factory_make("videoconvert", "vid_conv0");
    vid_conv1 = gst_element_factory_make("videoconvert", "vid_conv1");
    ...
    identity0 = gst_element_factory_make("queue","identity0");
    identity1 = gst_element_factory_make("queue","identity1");
    ...
    // g_object_set(compositor_pad0, "interpolation-method", 1,NULL);
    // g_object_set(compositor_pad1, "interpolation-method", 1,NULL);

    // g_object_set(vid_conv0, "interpolation-method", 1,NULL);
    // g_object_set(vid_conv1, "interpolation-method", 1,NULL);
    // g_object_set(sink_conv, "interpolation-method", 1,NULL);
    ...
    gst_bin_add_many(GST_BIN (pipline),
                    source0,identity0,
                    source1,identity1,
                    compositor,queues,sink_conv,sink,NULL);


    GstCaps *caps_src0;// UYVY ,framerate=60/1 UYVY
    caps_src0 = gst_caps_from_string("video/x-raw,width=1920,height=1080, format=UYVY");
    if (!gst_element_link_filtered(source0, identity0, caps_src0)) {
    ...
    GstCaps *caps_src1;
    caps_src1 = gst_caps_from_string("video/x-raw,width=1920,height=1080, format=UYVY");
    if (!gst_element_link_filtered(source1, identity1, caps_src1)) {
        g_printerr("Fail to gst_element_link_filtered videosrc -- vidconv\n");
        return -1;
    }
    ...
    // GstCaps *caps_conv0;
    // caps_conv0 = gst_caps_from_string("video/x-raw, width=1920, height=1080, format=UYVY");
    // // caps_conv0 = gst_caps_from_string("video/x-raw, width=1920, height=1080, format=UYVY,");
    // if (!gst_element_link_filtered(vid_conv0, identity0, caps_conv0)) {
    ...
    // GstCaps *caps_conv1;
    // caps_conv1 = gst_caps_from_string("video/x-raw, width=1920, height=1080, format=UYVY");
    // // caps_conv1 = gst_caps_from_string("video/x-raw, width=1920, height=1080, format=UYVY");
    // if (!gst_element_link_filtered(vid_conv1, identity1, caps_conv1)) {
    //     g_printerr("Fail to gst_element_link_filtered vidconv -- videoenc\n");
    //     return -1;
    // }
    ...
    sink_conv_caps = gst_caps_from_string("video/x-raw(memory:NVMM), width=1920, height=1080, format=NV12,framerate=30/1");
    ...
    src_pad0 = gst_element_get_static_pad(identity0, "src");// TODO: use vidconv
    src_pad1 = gst_element_get_static_pad(identity1, "src");
