...
{
    GstElement *pipline0, *pipline1, *source0, *sink0, *nvvidconv0,
    *source1, *sink1, *nvvidconv1;

    GstElement *queue0, *queue1;
    GstBus *bus0;
    GstMessage *msg0;
    GstStateChangeReturn ret0;
    ...
    pipline0 = gst_pipeline_new("test-pipline0");
    // pipline1 = gst_pipeline_new("test-pipline1");

    // build the pipe 
    gst_bin_add_many(GST_BIN (pipline0),source0,nvvidconv0,queue0,sink0,source1,nvvidconv1,queue1,sink1,NULL);
    // gst_bin_add_many(GST_BIN (pipline1),source1,nvvidconv1,queue1,sink1,NULL);
    ...
    if (!gst_element_link_filtered(source0, nvvidconv0, caps_src0)) {
        g_printerr("Fail to gst_element_link_filtered videosrc -- vidconv0\n");
        return -1;
    }
    ...
    if (!gst_element_link_filtered(source1, nvvidconv1, caps_src1)) {
        g_printerr("Fail to gst_element_link_filtered videosrc -- vidconv1\n");
        return -1;
    }
    gst_caps_unref(caps_src1);


    // gst_element_link(nvvidconv0,queue0);
    // gst_element_link(nvvidconv1,queue1);


    GstCaps *caps_conv0;
    caps_conv0 = gst_caps_from_string("video/x-raw(memory:NVMM), width=960, height=1080, format=RGBA");
    ...
    GstCaps *caps_conv1;
    caps_conv1 = gst_caps_from_string("video/x-raw(memory:NVMM), width=960, height=1080, format=RGBA");
    ...
    g_object_set(sink1, "overlay",0,"overlay-x", 960, "overlay-y", 0, "overlay-w", 960, "overlay-h", 1080, NULL);


    ret0 = gst_element_set_state(pipline0,GST_STATE_PLAYING);

    if(ret0 == GST_STATE_CHANGE_FAILURE)
    {
        g_printerr("Unable to set the pipeline0 to the playing state.\n");
        gst_object_unref(pipline0);
        return -1;
    }

    // ret1 = gst_element_set_state(pipline1,GST_STATE_PLAYING);

    // if(ret1 == GST_STATE_CHANGE_FAILURE)
    // {
    //     g_printerr("Unable to set the pipeline0 to the playing state.\n");
    //     gst_object_unref(pipline1);
    //     return -1;
    // }
    ...
    while (true)
    {
        ...
        bus0 = gst_element_get_bus(pipline0);
        msg0 = gst_bus_timed_pop_filtered(bus0,10*GST_MSECOND,
        ...
        if (msg0 != NULL) 
        {
            ...
            switch (GST_MESSAGE_TYPE (msg0)) 
            {
                case GST_MESSAGE_ERROR:
                gst_message_parse_error (msg0, &err, &debug_info);
                g_printerr ("Error received from element %s: %s\n",
                    GST_OBJECT_NAME (msg0->src), err->message);
    ...
    gst_element_set_state (pipline0, GST_STATE_NULL);
    gst_object_unref (pipline0);

    // gst_object_unref (bus1);
    // gst_element_set_state (pipline1, GST_STATE_NULL);
    // gst_object_unref (pipline1);
    return 0;
